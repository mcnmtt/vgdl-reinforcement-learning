...
class VGDLParser(object):
    ...
    def _eval(self, estr):
        import vgdl.ontology as ontology

        # Costruisci uno scope con:
        # - simboli del modulo corrente (core.py): BasicGame, VGDLSprite, ecc.
        # - simboli dell'ontology: AimedAvatar, Missile, GravityPhysics, ecc.
        scope = {}
        scope.update(globals())
        scope.update(ontology.__dict__)

        try:
            # prova a valutare espressioni Python (es. 25, True, GravityPhysics, ecc.)
            return eval(estr, scope)
        except Exception:
            # se non è un'espressione, prova a risolvere il nome nello scope
            if estr in scope:
                return scope[estr]
            # altrimenti lascia la stringa (serve per chiavi/parametri testuali)
            return estr
    ...
    def _parseArgs(self, s,  sclass=None, args=None):
        ...
        if '=' not in sparts[0]:
            ...
            # cerca prima nelle classi definite in core.py
            if name in globals():
                sclass = globals()[name]
            else:
                # poi nell'ontology
                import vgdl.ontology as ontology
                if hasattr(ontology, name):
                    sclass = getattr(ontology, name)
                else:
                    raise ValueError(f"Unknown VGDL class: {name}")
