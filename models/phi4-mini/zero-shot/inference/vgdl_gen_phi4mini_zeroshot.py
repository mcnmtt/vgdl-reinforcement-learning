...
# Experiment Config
# ======================
# Set MAX_DESCRIPTIONS = 3 for the first smoke tests.
# Set MAX_DESCRIPTIONS = None to process the whole DESCRIPTION_DIR.
DESCRIPTION_DIR = Path("dataset/descriptions")
OUTPUT_DIR = Path("models/phi4-mini/zero-shot/vgdl")
MAX_DESCRIPTIONS: 3 | None = 3
MAX_NEW_TOKENS = 400
...
def build_prompt(game_description: str) -> str:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Zero-shot VGDL generation with Phi-4-mini-instruct."
    )
    parser.add_argument(
        "--description-file",
        default=None,
    ...
    parser.add_argument(
        "--download-only",
        action="store_true",
        help="Download the model to the Hugging Face cache and exit.",
    )
    parser.add_argument(
        "--max-new-tokens",
        type=int,
    ...
    parser.add_argument(
        "--no-4bit",
        action="store_true",
        help="Load without 4-bit quantization. This needs much more VRAM/RAM.",
    )
    parser.add_argument(
        "--gpu-max-memory",
        default="4GiB",
    ...
    parser.add_argument(
        ...
        help="Use eager on older/consumer GPUs if flash attention is unsupported.",
    )
    parser.add_argument(
        "--trust-remote-code",
        action="store_true",
        help=(
            "Use the model repository's custom code. Disabled by default because "
            "the installed Transformers release can load Phi natively."
        ),
    )
    parser.add_argument(
        "--prompt-mode",
        default="raw",
        choices=["raw", "chat"],
        help="Use raw prompt text like Gemma4/Ollama, or the model chat template.",
    )
...
def select_description_files(args: argparse.Namespace) -> list[Path]:
    if args.description_file:
        return [Path(args.description_file)]
...
def output_path_for(description_file: Path, args: argparse.Namespace) -> Path:
    if args.description_file:
        ...
        return Path(args.output_dir) / f"{description_file.stem}_vgdl_phi4mini.txt"
    return Path(args.output_dir) / f"{description_file.stem}_vgdl_phi4mini.txt"
...
def generate_vgdl(
    ...
    # The prompt ends with a VGDL prefix, like the Gemma4 zero-shot script.
    # HF generation returns only tokens after the prompt, so we restore the
    # prefilled prefix before cleanup.
    return VGDL_PREFIX + tokenizer.decode(generated_ids, skip_special_tokens=True)
...
def main() -> None:
    ...
    if not description_files:
        raise RuntimeError("No description files selected.")
